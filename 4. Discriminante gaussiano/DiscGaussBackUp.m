...
%         end
%         
%         testdata.predictionAltura(j) = estado;
% 
%         if estado == testdata.cardio(j)
%             cont = cont + 1;
%             if estado == 0
...
%         end
%         
%         testdata.predictionPeso(j) = estado;        
% 
%         if estado == testdata.cardio(j)
%             cont = cont + 1;
%             if estado == 1
...
% predictionPesAlt = nan(length(testdata.id), 1);
% prediction3Indep = nan(length(testdata.id), 1);
% prediction3Dep = nan(length(testdata.id), 1);
% 
% testdata = [testdata table(predictionEdad) table(predictionPesAlt) table(prediction3Indep) table(prediction3Dep)];
...
w0Enfermo = -0.5*meanEnfermo'*inv(covEnfermo)*meanEnfermo-0.5*log(abs(det(covEnfermo))) + log(prior(2));

%%

cont = 0;
for i = 1:length(testdata.id)
    sano = (1/((2*pi)^(3/2)*((det(covSano)))))*exp(-0.5*(NaiveTest(i,:))*inv(covSano)*(NaiveTest(i,:))');
    enfermo = (1/((2*pi)^3/2*((det(covEnfermo)))))*exp(-0.5*(NaiveTest(i,:))*inv(covEnfermo)*(NaiveTest(i,:))');
